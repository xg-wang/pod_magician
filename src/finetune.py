...
def _train(
    ...
    def generate_and_tokenize_prompt(data_point: dict[str, str]):
        ...
        if not train_on_inputs:
            pod_prompt = generate_prompt(data_point["instruction"], data_point["input"])
            tokenized_pod_prompt = tokenize(pod_prompt, add_eos_token=add_eos_token)
            pod_prompt_len = len(tokenized_pod_prompt["input_ids"])

            if add_eos_token:
                pod_prompt_len -= 1

            tokenized_full_prompt["labels"] = [-100] * pod_prompt_len + tokenized_full_prompt["labels"][
                pod_prompt_len:
